...
def data_sql_helper():
	try :
		conn = mysql.connector.connect(user='root',password='123456',host='127.0.0.1',port='3306',\
			database='zdb_udc')
		cursor = conn.cursor()
		count = cursor.execute('select id,storeId,day,traffics from zt_traffic_daily')
		# count = cursor.execute('select id,storeId,day,predictions from zt_traffic_prediction')
		results = cursor.fetchall()
	except BaseException as e:	#如果数据库调用出现问题则添加日志，关闭服务
		logger.error(e)
		os._exit(0) #直接退出程序
...
def data_loader(train=True,args=None):
	...
	#加载训练数据
		...
		return train_x,train_y,storeId
